...
export default class Quotation {
    ...
    private constructor(id: string, scheduleAt: Date, serviceType: string, specialRequests: string[], expiresAt: Date, priceBreakdown: PriceBreakdown, isRouteOptimized: boolean, stops: Stop[]) {
        // dummy info
        this.id = id;
    ...
    static create(quotationPayload: QuotationPayload): Quotation {
        let makeQuotation = () => {
            return new Quotation("ID", new Date(), "MOTO", ["sp1", "sp2"], new Date(), {total: "12", currency: "HKD"}, true, []);
        };
